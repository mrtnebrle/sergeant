...
if [[ -n "${TEST_FIFO_PATH:-}" && "$last" == "$TEST_FIFO_PATH" &&
      "$*" == *"${TEST_FIFO_STAT_PATTERN:-%u:%d:%i}"* &&
      ! -e "$TEST_FIFO_MARKER" ]]; then
  if output="$("$REAL_STAT" "$@" 2>/dev/null)"; then
    rm -f "$last"
    mkfifo "$last"
    "$REAL_CHMOD" "$TEST_FIFO_MODE" "$last"
    : > "$TEST_FIFO_MARKER"
    printf '%s\n' "$output"
    exit 0
  fi
...
export REAL_STAT="$real_stat" REAL_CHMOD="$real_chmod" REAL_PYTHON="$real_python"
...
recorded_executable="$TEST_ROOT/old install/sgt-interactive-worker"
printf '%s\n' "$recorded_executable" > "$worker_state/worker_executable"
chmod 644 "$worker_state/worker_executable"
printf -v recorded_command '%q %q %q %q' \
  "$recorded_executable" "$worker_state" "$TEST_ROOT/worktree" opencode
if bash -c 'source "$1"; _sgt_worker_command_matches "$2" "$3" "$4" "$5" "$6"' _ \
  "$ROOT_DIR/bin/_sgt-lib.sh" "$recorded_command" \
  "$TEST_ROOT/current/sgt-interactive-worker" "$worker_state" \
  "$TEST_ROOT/worktree" opencode; then
  printf 'accepted unsafe worker_executable as a pane termination candidate\n' >&2
  exit 1
fi
...
for fifo_reader in owned legacy paired; do
  fifo_marker="$TEST_ROOT/$fifo_reader-fifo-marker"
  case "$fifo_reader" in
    owned)
      fifo_path="$TEST_ROOT/owned-fifo-race"
      printf 'owned-value\n' > "$fifo_path"
      chmod 600 "$fifo_path"
      fifo_mode=600
      # shellcheck disable=SC2016  # Expanded by the bounded nested Bash.
      reader_command='_sgt_read_owned_file "$2"'
      reader_args=("$fifo_path")
      ;;
    legacy)
      fifo_path="$TEST_ROOT/legacy-fifo-race"
      printf 'legacy-value\n' > "$fifo_path"
      chmod 664 "$fifo_path"
      fifo_mode=664
      # shellcheck disable=SC2016  # Expanded by the bounded nested Bash.
      reader_command='_sgt_read_matching_legacy_pane_identity "$2" legacy-value'
      reader_args=("$fifo_path")
      ;;
    paired)
      fifo_owner="$TEST_ROOT/paired-fifo-owner"
      fifo_path="$TEST_ROOT/paired-fifo-race"
      printf 'paired-release\n' > "$fifo_owner"
      chmod 600 "$fifo_owner"
      ln "$fifo_owner" "$fifo_path"
      fifo_mode=600
      # shellcheck disable=SC2016  # Expanded by the bounded nested Bash.
      reader_command='_sgt_read_same_owned_files "$2" "$3"'
      reader_args=("$fifo_owner" "$fifo_path")
      ;;
  esac

  set +e
  run_bounded 40 env PATH="$fake_bin:$PATH" TEST_SYSTEM=Darwin \
    TEST_DARWIN_FD_MODE=1 TEST_FIFO_PATH="$fifo_path" \
    TEST_FIFO_MARKER="$fifo_marker" TEST_FIFO_MODE="$fifo_mode" \
    bash -c "source \"\$1\"; $reader_command" _ \
    "$ROOT_DIR/bin/_sgt-lib.sh" "${reader_args[@]}" >/dev/null 2>&1
  status=$?
  set -e
  if [[ "$status" -eq 124 ]]; then
    printf '%s owned-file reader blocked after a FIFO swap\n' "$fifo_reader" >&2
    exit 1
  fi
  if [[ "$status" -eq 0 ]]; then
    printf '%s owned-file reader accepted a FIFO swap\n' "$fifo_reader" >&2
    exit 1
  fi
  [[ -e "$fifo_marker" && -p "$fifo_path" ]]
  rm -f "$fifo_path"
done
